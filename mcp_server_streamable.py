"""
MCP Server using Streamable HTTP transport with OpenWeatherMap API

This server demonstrates:
1. Streamable HTTP transport instead of SSE (single endpoint, better scaling)
...
- Uses single /mcp endpoint instead of separate /sse and /messages endpoints
- Better HTTP infrastructure compatibility (load balancers, CDNs)
...
# Streamable HTTP server imports
...
server = Server("weather-tools-streamable")
...
async def main():
    """
    Main function to run the MCP server with Streamable HTTP transport.
    """
    # Create Streamable HTTP transport - single endpoint design
    streamable_transport = StreamableHTTPServerTransport(
        mcp_session_id=None,  # Allow any session
        is_json_response_enabled=False,  # Use SSE streaming for responses
        event_store=None  # No event persistence needed for this example
    )

    async def app(scope, receive, send):
        """ASGI application for Streamable HTTP MCP"""
        if scope["type"] == "http":
            path = scope["path"]
            method = scope["method"]
    ...
    print("🌤️  Weather MCP Server (Streamable HTTP) starting...")
    ...
    print("⚠️  Single endpoint design - all communication via /mcp")
    ...
    async with streamable_transport.connect() as (read_stream, write_stream):
        ...
        async with asyncio.TaskGroup() as tg:
            # Run the HTTP server
            tg.create_task(server_instance.serve())
            ...
            tg.create_task(server.run(
                ...
                InitializationOptions(
                    server_name="weather-tools-streamable",
